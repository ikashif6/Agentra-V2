    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="relative w-64">
          ...
          <Input placeholder="Search by name or email…" className="pl-9 focus-visible:ring-[#E8470A]"
            value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        {isOwner && (
          <Button onClick={() => setInviteOpen(true)} style={{ background: "#E8470A" }}>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 gap-2 text-sm text-gray-400">
            ...
            {isOwner && !search && (
              <Button onClick={() => setInviteOpen(true)} className="mt-2" style={{ background: "#E8470A" }}>
              ...
              <div key={u._id} className="grid grid-cols-[2.5fr_2.5fr_1fr_1fr] gap-4 items-center px-5 py-3.5 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-3 min-w-0">
                  <Avatar className="h-8 w-8 shrink-0">
                    <AvatarFallback className="text-xs font-bold" style={{ background: "#FFF0EB", color: "#E8470A" }}>
                      {initials(u)}
                    </AvatarFallback>
                  </Avatar>
          ...
          <form onSubmit={handleSubmit(onInvite)} className="space-y-4 mt-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input {...register("firstName")} placeholder="Jane" className="focus-visible:ring-[#E8470A]" />
                {errors.firstName && <p className="text-xs text-red-500">{errors.firstName.message}</p>}
              </div>
              <div className="space-y-1">
                <Label>Last name</Label>
                <Input {...register("lastName")} placeholder="Doe" className="focus-visible:ring-[#E8470A]" />
            ...
            <div className="bg-orange-50 border border-orange-100 rounded-lg px-3 py-2.5 text-xs text-orange-700">
              An invite email will be sent. They&apos;ll set their password on first login.
            </div>
            <div className="flex justify-end gap-2 pt-1">
              <Button type="button" variant="outline" onClick={() => setInviteOpen(false)}>Cancel</Button>
              <Button type="submit" disabled={inviting} style={{ background: "#E8470A" }}>
