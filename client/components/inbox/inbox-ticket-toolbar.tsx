...
export function InboxTicketToolbar({
  ticket,
  currentUser,
  agents,
  teams,
    ...
    <div className="flex flex-wrap items-center gap-1.5 border-b border-border/60 px-3 py-2">
            ...
            {filteredAgents.map((agent) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="truncate text-sm">{agentName(agent)}</p>
      ...
      {ticket.inboxFolder === "snoozed" ? (
        ...
        >
          ...
          <span className="text-xs">Unsnooze</span>
        </Button>
      ) : (
        <Button variant="outline" size="sm" className="h-8 px-2.5" disabled={busy} onClick={onSnooze}>
      ...
      <DropdownMenu onOpenChange={(open) => !open && setTransferSearch("")}>
        ...
        <DropdownMenuContent align="end" className="w-52 p-1">
          ...
          <DropdownMenuItem className="gap-2.5 px-3 py-2" onClick={onMarkSpam}>
            <SpamIcon className="size-4 text-muted-foreground" />
            Mark as spam
          </DropdownMenuItem>
          <DropdownMenuSeparator />
